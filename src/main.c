...
"    card)\n"
"      if (( CURRENT == 3 )); then\n"
"        _values 'card subcommand' info profiles notifications deliver install enable disable delete\n"
"      else\n"
"        _arguments -s \\\n"
...
"      COMPREPLY=($(compgen -W \"info profiles notifications deliver install enable disable delete\" -- \"$cur\"))\n"
"    else\n"
...
usage(void) {
    fputs(
        ...
        "       euicc card info|profiles|notifications|deliver|install|enable|\n"
        ...
        "\n"
        "card deliver runs the cycle SGP.22 has an LPA run: retrieve\n"
        "each pending notification, send it to the address it names, and\n"
        "only then tell the eUICC to drop it. Nothing is removed that was\n"
        "not delivered -- the eUICC keeps no second copy. --server\n"
        "overrides the address each notification names, for testing\n"
        "against a server the card was never told about. Its exit code is\n"
        "0 when nothing failed, 1 when a delivery did, 2 when the card\n"
        "could not be asked.\n"
        "\n"
        "card notifications lists what the eUICC still has to have\n"
        "delivered. Every profile management operation leaves an entry,\n"
        "and the eUICC keeps it until an LPA delivers it to the address\n"
        "the entry names and then says so -- nothing here does that yet,\n"
        "so the list is a history rather than a backlog. Its exit code\n"
        "follows card profiles' below.\n"
        "\n"

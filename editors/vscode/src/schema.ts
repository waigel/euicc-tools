...
/*
 * Laying a file out used to live here, in a second lexer written in TypeScript.
 * It is `euicc fmt` now, for the reason everything else in this project follows:
 * a second reading of the language is a second thing that can disagree with the
 * first. That one did -- it shortened a cstring running over two lines, and its
 * own check could not see the change because the same duplicate got the X.680
 * clause wrong.
 */
